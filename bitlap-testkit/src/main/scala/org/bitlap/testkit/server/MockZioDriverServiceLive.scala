 ...
 */
case class MockZioDriverServiceLive(private val zioRpcBackend: RpcN[ZIO])
...
object MockZioDriverServiceLive {
  ...
  val mockLiveWithBackend: UIO[MockZioDriverServiceLive] = ZIO.succeed(new MockZioDriverServiceLive(ZioRpcBackend()))

  val mockLive: UIO[MockZioDriverServiceLive] = ZIO.succeed(new MockZioDriverServiceLive(ZioRpcBackend()))
}
