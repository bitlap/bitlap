...
package org.bitlap.server.raft.storage
...
abstract class RocksDBSupport(dbRef: Ref[RocksDB], serdeRef: Ref[Serde]) {

  def withDB[T](f: RocksDB => T): IO[StorageException, T] =
    for {
      rocksDB <- dbRef.get
      bytes   <- ZIO.fromTry(Try(f(rocksDB))).mapError(e => StorageException("Command to RocksDB failed", Option(e)))
    } yield bytes

  private def deserialize[T: ClassTag](bytes: Array[Byte]): IO[SerializationException, T] =
