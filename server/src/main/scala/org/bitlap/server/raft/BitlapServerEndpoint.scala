...
import org.bitlap.network.processor._
import org.bitlap.network.session.SessionManager
import org.bitlap.network.{ NetworkHelper, NetworkServiceImpl }
 ...
 */
class BitlapServerEndpoint(private val conf: BitlapConf)
    extends LifeCycleThread("BitlapServerEndpoint")
    with NetworkHelper {
  ...
  override def run(): Unit = {
    ...
    registerMessageInstances(
      ...
      (t1, t2) => RpcFactoryHelper.rpcFactory().registerProtobufSerializer(t1, t2)
    )
    registerMessageInstances(
  ...
  private def registerProcessor(rpcServer: RpcServer): Unit = {
    ...
    ).foreach(rpcServer.registerProcessor)
  }
}
