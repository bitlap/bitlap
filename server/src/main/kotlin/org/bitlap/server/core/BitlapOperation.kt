...
class BitlapOperation(parentSession: Session, opType: OperationType, hasResultSet: Boolean = false) :
    ...
    private fun wrapper(rs: ResultSet): models.QueryResult {
        rs.use {
            // get schema
            ...
            val columns = (1..metaData.columnCount).map {
                ...
                val colType = when (metaData.getColumnType(it)) {
                    Types.VARCHAR -> models.TypeId.B_TYPE_ID_STRING_TYPE
                    Types.INTEGER -> models.TypeId.B_TYPE_ID_INT_TYPE
                    Types.DOUBLE -> models.TypeId.B_TYPE_ID_DOUBLE_TYPE
                    // TODO more
                    else -> models.TypeId.B_TYPE_ID_UNSPECIFIED
                }
            ...
            return models.QueryResult(TableSchema(columns), RowSet(rows))
        }
    }
}
