...
//      val readerContext = CsvReaderContext()
//      readerContext.setSkipEmptyLine(true)
//      CsvReader(readerContext).open(input)
...
//        }
    ...
    private def write0(events: List[Event]): Unit = {
        events.groupBy(_.getTime).foreach { case (time, rs) =>
            // 1. agg metric
            val singleRows = rs
              .groupBy { it => (it.getEntity, it.getDimension, it.getMetric.getKey) }
              .map { case ((entity, dimension, metricKey), groupRows) =>
            ...
            // store metrics
            val metricRows = cleanRows.groupBy { it => (it.getEntity.getKey, it.getMetric.getKey) }
              .map { case ((entityKey, metricKey), groupRows) =>
                val bbm = BBM()
                val cbm = CBM()
                ...
                MetricRow(time, metricKey, cbm, bbm, MetricRowMeta(time, metricKey, bbm.getCountUnique, bbm.getLongCount, cbm.getCount))
              }
              .toList
            ...
            val metricDimRows = cleanRows
                ...
                .groupBy { it => (it.getEntity.getKey, it.getMetric.getKey, it.getDimension.firstPair()) }
              .map { case ((entityKey, metricKey, dimension), groupRows) =>
                ...
                MetricDimRow(time, metricKey, dk, d, cbm, bbm, MetricDimRowMeta(time, metricKey, dk, d, bbm.getCountUnique, bbm.getLongCount, cbm.getCount))
              }
              .toList
