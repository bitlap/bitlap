...
class BitlapParquetWriter[T](
    ...
    private val outputFile = Path(output, BitlapWriters.genUniqueFile(txId, "mdm", s"${CompressionCodecName.SNAPPY.getExtension}.parquet"))
    ...
    override def writeBatch(rows: Iterable[T]): Unit = {
        ...
        initWriter()
        rows.foreach { row =>
            this.writer.write(this.rowConvert(row))
        }
    }
    ...
    private def initWriter(): Unit = {
        if (this.writer != null) {
            scala.util.Using.resource(this.writer)
            this.writer = null
        }
