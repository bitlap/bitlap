        ...
        val container = metricType match {
          case c if c == classOf[DataTypeRowValueMeta] => {
                MDContainer[MetricRowMeta, RowValueMeta](1).also { container =>
                  store.queryMeta(timeFilter, metrics).asScala
                    .foreach { row =>
                      container.put(
                        row.tm, row,
          ...
          case c if c == classOf[DataTypeRBM] => {
                MDContainer[MetricRow, RBM](1).also { container =>
                  store.queryBBM(timeFilter, metrics).asScala
                    .foreach { row =>
                      container.put(
                        row.tm, row,
                        { it => it.entity.getRBM() },
          ...
          case c if c == classOf[DataTypeCBM] => {
                MDContainer[MetricRow, CBM](1).also { container =>
                  store.queryCBM(timeFilter, metrics).asScala
                    .foreach { row =>
                      container.put(
                        row.tm, row,
                        { it => it.metric },
        ...
        val container = metricType match {
          case c if c == classOf[DataTypeRowValueMeta] => {
                MDContainer[MetricDimRowMeta, RowValueMeta](2).also { container =>
                    store.queryMeta(timeFilter, metrics, dimension, dimensionFilter).asScala
                        .foreach { row =>
                            container.put(
                                List(row.tm, row.dimension), row,
                                { it => RowValueMeta.of(it.entityUniqueCount, it.entityCount, it.metricCount) },
                                { (a, b) => a.add(b.entityUniqueCount, b.entityCount, b.metricCount) }
                            )
                        }
          ...
          case c if c == classOf[DataTypeRBM] => {
                MDContainer[MetricDimRow, RBM](2).also { container =>
                    store.queryBBM(timeFilter, metrics, dimension, dimensionFilter).asScala
                        .foreach { row =>
                            container.put(
                                List(row.tm, row.dimension), row,
                                { it => it.entity.getRBM() },
          ...
          case c if c == classOf[DataTypeCBM] => {
                MDContainer[MetricDimRow, CBM](2).also { container =>
                    store.queryCBM(timeFilter, metrics, dimension, dimensionFilter).asScala
                        .foreach { row =>
                            container.put(
                                List(row.tm, row.dimension), row,
                                { it => it.metric },
