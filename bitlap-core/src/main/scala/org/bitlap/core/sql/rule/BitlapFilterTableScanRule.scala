...
class BitlapFilterTableScanRule extends AbsRelRule(classOf[BitlapFilter], "BitlapFilterTableScanRule") {
  ...
  private def pruneFilter(
    ...
    val timeField  = rowType.getFieldList.asScala.find(_.getName == Keyword.TIME).get
    ...
    val unPruneFilter = filter.accept(new RexShuttle() {
      override def visitCall(call: RexCall): RexNode = {
        call.getKind match {
          ...
          case k if SqlKind.COMPARISON.contains(k) || k == SqlKind.SEARCH =>
            val left  = call.operands.get(0)
            val right = call.operands.get(1)
            (left, right) match {
              case (l, r)
                  if (l.isInstanceOf[RexInputRef] && r.isInstanceOf[RexLiteral])
                    || (l.isInstanceOf[RexLiteral] && r.isInstanceOf[RexInputRef]) =>
