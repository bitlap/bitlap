          ...
          case scan: BitlapTableScan => {
                ...
                if (oFilter.isAlwaysFalse) {
                    BitlapTableFilterScan(
                        scan.getCluster, scan.getTraitSet, scan.getHints, scan.getTable,
            ...
            override def visitCall(call: RexCall): RexNode = {
                return call.getKind match {
                  ...
                  case k if k == SqlKind.AND => {
                        val operands = ListBuffer[RexNode]()
                            .also { it => visitList(call.operands, it.toList.asJava) }
                            .filter(_ != null)
                        if (operands.size == 2) {
    ...
    def resolveFilter[T](
        ...
        val filter = RexUtil.apply(
            Mappings.target(
              ...
              Int.box(inputField.getIndex + 1), Int.box(1)
            ),
            inputFilter
        )
