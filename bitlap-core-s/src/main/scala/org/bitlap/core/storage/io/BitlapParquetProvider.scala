...
import org.bitlap.core.storage.io._
  ...
  import BitlapParquetProvider._
    ...
    override def getMetricWriter(output: Path): BitlapWriter[MetricRow] = {
        return BitlapParquetWriter(fs, output, METRIC_SCHEMA, ParquetWriter.DEFAULT_BLOCK_SIZE / 8L, { row =>
          ...
          r.put(4, JsonEx.json(row.metadata))
          r
        }
      )
    }
    ...
    override def getMetricDimWriter(output: Path): BitlapWriter[MetricDimRow] = {
        return BitlapParquetWriter(fs, output, METRIC_DIM_SCHEMA, ParquetWriter.DEFAULT_BLOCK_SIZE / 4L, { row =>
            val r = GenericData.Record(METRIC_DIM_SCHEMA)
                ...
                r. put(6, JsonEx.json(row.metadata))
                r
            }
        )
    }

    override def getMetricMetaReader(dataPath: Path, timeFunc: TimeFilterFun, metrics: List[String], projections: List[String]): BitlapReader[MetricRowMeta] = {
        ...
        return BitlapParquetReader(fs, inputs, METRIC_SCHEMA, requestedProjection, metricFilter.compact(), { row =>
            val metaObj = JsonEx.jsonAs(row.getWithDefault("meta", "{}"), classOf[MetricRowMeta])
    ...
    override def getMetricReader(dataPath: Path, timeFunc: TimeFilterFun, metrics: List[String], projections: List[String]): BitlapReader[MetricRow] = {
        val inputs = this.listFilePath(dataPath, timeFunc)
        val metricFilter = BitlapReaders.makeParquetFilter("mk", metrics)
        val requestedProjection = BitlapReaders.makeAvroSchema(METRIC_SCHEMA, projections)
        return BitlapParquetReader(fs, inputs, METRIC_SCHEMA, requestedProjection, metricFilter.compact(), { row =>
    ...
    override def getMetricDimMetaReader(
        ...
        return BitlapParquetReader(fs, inputs, METRIC_DIM_SCHEMA, requestedProjection, filter.compact(), { row =>
            val metaObj = JsonEx.jsonAs(row.getWithDefault("meta", "{}"), classOf[MetricDimRowMeta])
            if (timeFunc(metaObj.tm)) {
                metaObj
            } else {
                null
    ...
    private def getMetricDimFilter(metrics: List[String], dimension: String, dimensionFilter: PrunePushedFilter): FilterPredicate = {
    ...
    private def listFilePath(dataPath: Path, timeFunc: TimeFilterFun): List[Path] = {
        return fs.listStatus(dataPath)
            .filter(it =>
                it.isDirectory && timeFunc(it.getPath.getName.split("=")(1).toLong)
            )
            .flatMap(filePath =>
                fs.listStatus(filePath.getPath, HiddenFileFilter.INSTANCE).map(_.getPath)
            ).toList
    }
}
...
object BitlapParquetProvider {
  ...
  val METRIC_SCHEMA: Schema = SchemaBuilder.builder()
    .record("metric").namespace(classOf[BitlapParquetProvider].getPackageName
  ).fields()
    .optionalString("mk")
    .optionalLong("t")
    // .optionalString(ek)
